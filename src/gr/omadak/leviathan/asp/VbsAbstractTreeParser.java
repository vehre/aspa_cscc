...
public abstract class VbsAbstractTreeParser extends antlr.TreeParser implements
		TreeVbsTokenTypes, SymbolTableExposer {
...
//				+ parser.getCurrentFileName().replace('.', '_'));
	...
	protected AST translateMember(Member mem, ASPObjectInstance instance) {
		...
		if (mem instanceof Property) {
			...
			result = prop
					.read(instance == null ? null : instance.getInstance());
		} else if (mem instanceof Method) {
			Method method = (Method) mem;
			result = method.translate(instance == null ? null : instance
					.getInstance());
		} else {
			result = null;
	...
	protected AST translateMember(Member mem, ASPObjectInstance instance,
		...
		if (mem instanceof Property) {
			...
			result = prop.write(instance == null ? null : instance
					.getInstance());
			if (result != null) {
			...
			if (property != null) {
				...
				log.error("Can not assign to a method who does not have"
	...
	protected AST getDefault(AST ast) {
		...
		if (ast != null) {
			if (ast.getType() == IDENTIFIER && currentFunction != null
				...
				currentFunction
						.placeVar(name, getIdentifierType(name), !isGlob);
			}
	...
	protected ASPObjectInstance getMethod(AST ast, List args) {
		...
		if (ast instanceof ObjectAST) {
			...
			if (obj instanceof Method) {
				...
				if (clazz == null) {
					log.error("Expected method but found property:"
							+ obj.getName()
							+ " wich does not evaluate to class");
				} else {
					result = new ASPObjectInstance(instance, clazz);
		...
		} else if (ast.getType() == IDENTIFIER || ast.getType() == METHOD_CALL) {
			...
			if (inGlobFunctions || inLocalFunctions) {
				result = setMember(
						inGlobFunctions ? FUNCTIONS : localFunctions,
	...
	protected Method getSuitableMethod(List methods, List args) {
		...
		for (Iterator it = methods.iterator(); it.hasNext();) {
			...
			if (mem instanceof Method) {
				...
				if (methArgs.size() == args.size()
						|| (methArgs.size() - 1 < args.size() && methArgs
								.contains(ALLARGS))) {
					comply.add(method);
				}
			}
		...
		default:
			log.warn("More then 2 alternatives for method:"
					+ ((ASPObject) comply.get(0)).getName() + " for args:"
					+ args);
			result = (Method) comply.get(0);
			break;
		}
	...
	protected void translateUntil(AST uExpr) {
		...
		case NOT:
			...
			break;
		default:
			dw = (AST) astFactory.make((new ASTArray(2)).add(
					astFactory.create(NOT, "not")).add(dw));
			/* dw = #([NOT, "not"], dw); */break;
		}
	...
	protected void placeObject(String name, ASPObjectInstance inst) {
		if (currentFunction != null) {
			currentFunction.placeObject(name, inst, !identifiers.contains(name)
	...
	protected void addVariable(String name, int type) {
		if (currentFunction != null) {
			boolean local = currentFunction.hasVar(name)
	...
	protected AST handleIdObject(AST id, ObjectAST obj) {
		...
		int retType = -1;
		if (obj.getType() == CommonConstants.OBJECT_RET
				&& aInst.getMember() == null) {
			ASPClass clazz = aInst.getASPClass();
			result = aInst.getInstance();
			if (result == null && clazz != null && clazz.hasConstructor()) {
				result = translateMember((Method) clazz.getConstructors()
						.get(0), null);
			}
			placeObject(id.getText(), new ASPObjectInstance(astFactory.dup(id),
					aInst.getASPClass()));
			retType = 0;
		} else {
			...
			if (member != null) {
				if (member.getRetObjectClass() != null) {
					placeObject(
							id.getText(),
							new ASPObjectInstance(astFactory.dup(id), member
									.getRetObjectClass()));
					retType = 0;
	...
	protected AST handleWriteToObject(ObjectAST ast, AST value) {
		...
		if (instance != null) {
			if (instance.getMember() != null) {
				result = translateMember(instance.getMember(), instance, value);
			} else {
				ASPClass clazz = instance.getASPClass();
				Member mem = clazz.getDefaultProperty() != null ? (Member) clazz
						.getDefaultProperty() : (Member) clazz
						.getDefaultMethod();
				if (mem != null) {
					result = translateMember(mem, instance, value);
				} else {
	...
	protected AST translateObject(AST ast) {
		...
		return instance == null ? ast
				: translateObject(instance, ast.getType());
	}
	...
	protected void addProperty(Map props, AST root, int index) {
		...
		if (mem instanceof VbUserDefinedProperty) {
			...
			switch (index) {
			case 0:
				newName = property.getGet().getName();
				break;
			case 1:
				newName = property.getLet().getName();
				break;
			default:
				newName = property.getSet().getName();
				break;
			}
	...
	protected VbUserDefinedMethod getProperty(String name, int type) {
		VbUserDefinedProperty prop = (VbUserDefinedProperty) currentClass
